...
- `cold-item/`
  - baseline для решения проблемы `cold-item`
  - его имеет смысл использовать, если основная задача связана именно с новыми или редкими объектами
...
- `cold-user/`
  ...
  - объединяет в себе решение для:
    ...
    - `cold-user`
    - `global cold-start`
  - именно эту ветку рекомендуется использовать как основное решение проблемы `cold-start`
